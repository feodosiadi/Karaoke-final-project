...
async function extractFeatures(buffer) {
  ...
  // Преобразуем buffer в Float32Array, если он еще не в этом формате
...
async function compareFeatures(recordFeatures, acapellaFeatures) {
  ...
  const maxLogEnergy = Math.max(Math.log(recordFeatures.energy), Math.log(acapellaFeatures.energy)) || 1;
  ...
  const maxCentroid = Math.max(recordFeatures.spectralCentroid, acapellaFeatures.spectralCentroid) || 1;
  ...
  const normalizedMfccDiff = 1 - Math.min(mfccDifference / (maxMfccDifference * recordFeatures.mfcc.length), 1);
...
songsRouter
  ...
  .post(upload.single('record'), async (req, res) => {
    try {
      const { songId } = req.params;
      if (!songId || Number.isNaN(Number(songId))) {
        return res.status(400).json({ message: 'Invalid id' });
